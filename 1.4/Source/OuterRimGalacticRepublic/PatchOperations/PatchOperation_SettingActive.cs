...
namespace OuterRimGalacticRepublic
{
    public class PatchOperation_SettingActive : PatchOperation
    {
        ...
        public override bool ApplyWorker(XmlDocument xml)
        {
            ...
            ModContentPack modContentPack = OuterRimGalacticRepublicMod.mod.Content;
            for (int i = 0; i < settings.Count(); i++)
            {
                if (!OuterRimGalacticRepublicMod.settings.IsValidSetting(settings[i]))
                {
                ...
                if (OuterRimGalacticRepublicMod.settings.GetEnabledSettings.Contains(settings[i]))
                {
                    flag = true;
                    break;
                }
